function CreateAccount(){
  ...
  const emailValidateFormat = email => {
    ...
    return 'Please enter a valid email format similar to "my@domain.com" or "my@domain.co.uk"'; // 'Username should be an email';
  };
  ...
  function validate(field, label){
      ...
      if (label === 'email') {
        if (field.length < 5) {
          message = `${label.toUpperCase()} requies a real email address`;
          setStatus(`Error: ${message}`);
          alert(message);
          return false;
        } else {
